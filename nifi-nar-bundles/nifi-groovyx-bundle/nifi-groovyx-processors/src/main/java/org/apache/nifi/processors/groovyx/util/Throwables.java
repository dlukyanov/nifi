...
public class Throwables{
	...
	public static String getMessage(Throwable e,Object priority, int maxlen){
		...
		if(priority!=null){
			if(priority instanceof String){
				...
				if( !(priority instanceof Class) )priority=priority.getClass();
		...
		if(traceIndex==-1){
			for(int i=0;i<trace.length;i++){
				...
				if(		cl.startsWith("java.") ||
						cl.startsWith("javax.") || 
						cl.startsWith("org.omg.") || 
						cl.startsWith("org.w3c.") || 
						cl.startsWith("org.xml.") ||
						cl.startsWith("groovy.lang.") ||
						cl.startsWith("groovy.util.") ||
						cl.startsWith("org.codehaus.") ||
						cl.startsWith("com.springsource.") ||
						cl.startsWith("org.springframework.") ||
						cl.startsWith("org.apache.") ||
						cl.startsWith("sun.")||
						cl.startsWith("com.sun.")||
						cl.startsWith("org.junit.")||
						cl.startsWith("junit.framework.")
		...
		if(maxlen>0 && msgSuffix.length()+msg.length()>maxlen) {
			if(maxlen>msgSuffix.length()+2){
				...
				if(newlen<msg.length())msg = msg.substring(0,newlen);
				msg = msg + ".." + msgSuffix;
	...
	private static Throwable getRootException(Throwable e){
		...
		*/
		...
		if(t!=null)return getRootException(t);
		return e;
	}
}
